...
export default function Index({ mentordetails }) {
    return (
        <AuthLayout>
            ...
            <h1 className="text-2xl font-bold mb-4 text-center">Daftar Mentor</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mentor-list">
                {mentordetails.map((mentor) => (
                    <div key={mentor.id} className="mentor-card bg-white shadow-lg rounded-lg flex flex-col">
                        ...
                        <Link href={` /mentors/${mentor.slug}`} className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-b-lg text-center text-white p-2">
                            Lihat Lebih Rinci
                        </Link>
                    </div>
                ))}
